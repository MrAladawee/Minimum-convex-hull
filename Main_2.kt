...
{/*Shape start*/
    ...
    Canvas(modifier = Modifier.fillMaxSize().clickable{}.onPointerEvent(PointerEventType.Press){/* oPE open*/
        if ((it.buttons.isSecondaryPressed)==false){
            if ((it.changes.first().position in Dataset_points) == false) {
                Dataset_points.add(it.changes.first().position)
                redraw = !redraw
            }
        }
        if (it.buttons.isSecondaryPressed){
            var p = it.changes.first().position
            var dp = mutableListOf<Offset>()
            for (p in Dataset_points){
                dp.add(p)
            }

            for (point in dp){
                var a = (p.x - point.x)*(p.x - point.x)
                var b = (p.y - point.y)*(p.y - point.y)
                if (a+b <= 100 ){
                    if (point in Dataset_points) {
                        Dataset_points.remove(point)
                    }
                }
            }
            redraw = !redraw
